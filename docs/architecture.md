...
Provider chain slugs are derived from the CoinGecko platform ID with a small alias table, and GeckoTerminal network slugs are discovered from its `/networks` endpoint because they differ (`eth` versus `ethereum`). CoinGecko remains in use for the chain list and public token lists, and the platform list also supplies native currency logos; the large `/coins/list` mapping call is no longer needed.
...
Native currencies have no token-list entry, so their logo cannot come from an address-keyed source, and CoinGecko's batched market endpoint is throttled from Workers egress. Instead, the platform list's `native_coin_id` is resolved against the platform IDs, which share CoinGecko's ID namespace: when a currency is itself a platform, that platform's image is used. This resolves ETH on every ETH-native L2 to the Ethereum coin image rather than the L2's chain logo, and it costs no additional upstream request. A currency that is not a platform falls back to its own platform's chain image, which is the chain's native brand. Platforms with neither leave the logo null rather than guessed. This keeps native identity as `chainId + native` and requires no hardcoded logo or symbol table.
