...
Provider chain slugs are derived from the CoinGecko platform ID with a small alias table, and GeckoTerminal network slugs are discovered from its `/networks` endpoint because they differ (`eth` versus `ethereum`). CoinGecko remains in use for the chain list, public token lists, and a batched native-coin image lookup; the large `/coins/list` mapping call is no longer needed.
...
Native currencies have no token-list entry, so their logo cannot come from an address-keyed source. Each platform's `native_coin_id` is resolved to that coin's own image through one batched CoinGecko call, cached for seven days, with the platform chain image as a fallback for coins the lookup cannot resolve or platforms that publish no coin ID. This keeps native identity as `chainId + native` and requires no hardcoded logo or symbol table. A failed lookup retains the cached mapping and never aborts a sync.
