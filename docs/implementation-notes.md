...
- A first attempt resolved each platform's `native_coin_id` through CoinGecko's batched `/coins/markets` endpoint, cached in `app_state`. Verified in production that keyless `/coins/markets` returns HTTP 429 to Cloudflare egress on every run, while `/asset_platforms` is allowed, so the coin image was never resolved and natives fell back to the chain image. Reproduced the same request from a non-Cloudflare host returning 200 with 183 of 217 coin ids resolved, confirming the failure is egress-specific rather than a schema or request problem. GeckoTerminal's native token exposes no image, and DefiLlama's address-keyed `token-icons.llamao.fi` icons cover only 27 of 275 chains, so neither replaced the CoinGecko lookup.
- Resolved it without an upstream call: CoinGecko platform IDs and coin IDs share a namespace, so a platform whose `id` equals a currency's `native_coin_id` carries that coin's logo. `discoverChains` now builds a platform-id image map and prefers it over the platform's own chain image. Effect verified in the data: `arbitrum-one`, `base`, `optimism`, `linea`, `scroll`, and other ETH-native L2s all declare `native_coin_id: "ethereum"`, so they resolve to the Ethereum coin image instead of their chain logo. 141 of 275 platforms resolve this way; the rest use their chain image, and a platform with neither stays null.
- Removed the `/coins/markets` call, its `app_state` cache, and the `nativeImages` plumbing entirely, so the feature adds no upstream request, no cache, and no throttling exposure. The platform schema still captures `image.large`, which the sync already fetches.
- Added a runtime test covering a currency whose coin is its own platform, an ETH-native L2 resolving to Ethereum's image (not its chain logo), the chain-image fallback with thumbnail-to-large normalization, and a null logo when neither is available.
- Updated the landing page, `docs/api.md`, and `docs/architecture.md`.
